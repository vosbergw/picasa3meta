...
class IniInfo(object):
	...
	Picasa3 creates a .picasa.ini file in each directory listing with the format:
	...
	def __init__(self,iniFile,contacts=None):
		...
		for line in inIni:
			...
			try:
				...
				if len(self.names) == 0:
					raise IniStructError("unexpected lines in %s before a file designator"%iniFile)
				else:  
					...
					if key == "faces" and contacts != None:
						...
						for people in val.split(';'):
							if sfaces != "sfaces:":
								sfaces += ","
							person = people.split(',')
							# people has the form 'rect(),id', so split that on the ','
							# and the id is person[1]
